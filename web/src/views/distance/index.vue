<template lang="pug">
  .app-container
    .filter-container
      ...
      el-button( :loading="downloadLoading" class="filter-item" type="primary" icon="el-icon-search" @click="handleCreate") 新建距离
      ...
      el-form(
        ...
        template(v-if="temp.local")
          el-form-item(required label="特征文件" prop="libraryId")
            el-select(v-model="temp.featureId" placeholder="请选择特征" style="width: 100%;")
              el-option(v-for="feature in features" :label="feature.name" :key="feature.id" :value="feature.id")
          el-form-item(:label="'距离算法'" prop="feature" )
...
export default {
  ...
  methods: {
    queryFeatures() {
      this.temp.featureId = ''
      featureService.get({ libraryID: this.temp.libraryId }).then(res => {
        this.features = res.data.items
      })
    },
    getList() {
      this.listLoading = true
      distanceService.get(this.listQuery).then(res => {
    ...
    createData() {
      distanceService.create(this.temp).then(res => {
