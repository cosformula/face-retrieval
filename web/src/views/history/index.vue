  ...
  .app-container
    ...
    el-table.table(v-loading='listLoading', :key='tableKey', :data='list', border='', fit='', highlight-current-row='')
      el-table-column(:label="'序号'", align='center', width='325')
        template(slot-scope='scope')
          span(style="overflow:hidden;white-space:nowrap;") {{ scope.row.id }}
    ...
    el-dialog(:title='textMap[dialogStatus]', :visible.sync='dialogFormVisible')
      el-form(ref='dataForm', :rules='rules', :model='temp', label-position='left', label-width='70px', style='width: 400px; margin-left:50px;')
        el-form-item(:label="$t('table.type')", prop='type')
          el-select.filter-item(v-model='temp.type', placeholder='Please select')
        ...
        el-form-item(:label="$t('table.status')")
          el-select.filter-item(v-model='temp.status', placeholder='Please select')
    ...
    el-dialog(:visible.sync='dialogPvVisible', title='Reading statistics')
      ...
      span.dialog-footer(slot='footer')
        el-button(type='primary', @click='dialogPvVisible = false') {{ $t('table.confirm') }}
</template>

<script>
import { fetchRetrieves } from '@/api'
...
export default {
  ...
  methods: {
    getList() {
      ...
      fetchRetrieves(this.listQuery).then(response => {
